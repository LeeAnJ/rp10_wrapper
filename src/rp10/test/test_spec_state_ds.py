...
from calc_ds import calc_ds_and_compare_with_sample


def test_spec_state_ds(mixture):

    # print output data into prettytable: set table instance and table header
    table_1 = PrettyTable(padding_width=2)
    col1_header = "Spec.-state f(d,s,x): butane-ethane-methane"
    col2_header = "EPSmin, %"
    col3_header = "EPSmax, %"
    ...
    data_list = calc_ds_and_compare_with_sample(fluid=mixture,
                                                d=d, d_units=d_units,
                                                s=s, s_units=s_units,
                                                x=x, x_units=x_units)
