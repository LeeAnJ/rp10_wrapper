...
from calc_ps import calc_ps_and_compare_with_sample

def test_spec_state_ps(mixture):

    # print output data into prettytable: set table instance and table header
    table_1 = PrettyTable(padding_width=2)
    col1_header = "Spec.-state f(p,s,x): butane-ethane-methane"
    col2_header = "EPSmin, %"
    col3_header = "EPSmax, %"
    ...
    # calc. f(p,s,x) for subcooled liquid phase and compare results with sample
    data_list = calc_ps_and_compare_with_sample(fluid=mixture,
                                                p=p, p_units=p_units,
                                                s=s, s_units=s_units,
                                                x=x, x_units=x_units)
